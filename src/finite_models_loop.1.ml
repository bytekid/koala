...
(* check here *)
  ...
  let prep_clauses =  
    Preprocess.preprocess ~is_eq_problem:(Problem_properties.has_equality input_clauses)  input_clauses in
...
(* adjust options; TODO: move this later *)
  ...
  then (* there are no types that need to be extended (all types non-flattend) *)
    (
     ...
     let prover_functions = Proof_search_loop.create_provers_current_options !current_clauses in
     Proof_search_loop.full_loop prover_functions ~time_limit:Undef current_clauses
    )
  else
    begin
(*--------------main loop ------------------*)
  ...
  do
	...
	Finite_models.domain_axioms_triangular new_dom_pred !dom_const_list
      ...
      let lemmas = get_lemmas fm_state in 
      
      dbg D_lemmas (lazy "\n\n ---------- Lemmas -------- \n\n");
      dbg D_lemmas (lazy (Clause.clause_list_to_string lemmas));

(*      out_warning (" finite models lemmas are switched off ");*)


      let clauses_before_prep = (lemmas@axioms@init_clauses) in
	...
	then 
	  begin
	    ...
	    Prop_solver_exchange.assign_adjoint_preds [];
...
(*      let neg_bound_pred = add_neg_atom new_bound_pred in   *)
      ...
      Prop_solver_exchange.assign_adjoint_preds adjoint_assumptions_list;
...
(*      List.iter Prop_solver_exchange.add_clause_to_solver !clauses_ref; *)
	...
	raise Unsatisfiable_gr
      ...
      Proof_search_loop.full_loop prover_functions ~time_limit:Undef clauses_ref
...
(*    |Unsatisfiable_gr_na -> failwith "dbg: finite_models_loop Unsatisfiable_gr_na" *)
	...
	let get_unsat_core () = 
      ...
      let unsat_core_clauses = Prop_solver_exchange.get_unsat_core_clauses uc in
