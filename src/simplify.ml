...
(* inconsistent with solver assumptions excluding soft assumptions  *)
let inconsistent_with_assumptions clause = 
  if not (Prop_solver_exchange.is_empty_assumptions ()) 
  then 
    (
     let lits = Clause.get_lits clause in 
    
     if (consistent_with_assumptions_some_lits lits)
     then () (* not empty modulo assumptions*)
     else (* empty modulo assumptions *)
       (
        Prop_solver_exchange.add_clause_to_solver clause;
...
let pre_cond_true_fun = SubsumptionIndex.pre_cond_true_fun 
...
let forward_subs ?(pre_cond = pre_cond_true_fun)  sim_state clause = 
  ...
  match
    (SubsumptionIndexM.is_subsumed ~pre_cond
       sim_state.sim_sub_index clause)
  with
  | Some((by_cl, _subst))  -> 
      (
...
let forward_subs_strict sim_state clause = 
  ...
  with 
    Eliminated ->
      ( dbg D_fwd_subs_strict (lazy (" subsumed: "^(Clause.to_string clause)));
        raise Eliminated
       )
  ...
  then
    (
     ...
     List.iter (fun subsumed ->       
       ...
       dbg D_bwd_subs (lazy ((Clause.to_string subsumed)^" by: "^(Clause.to_string sim_clause)^(" Claus.bc_euqal:  ")^(string_of_bool (Clause.equal_bc subsumed sim_clause))));
