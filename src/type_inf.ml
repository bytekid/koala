...
type prop_pred_ext = 
    {
     prop_pred_ext : symbol;

(* pos extention *)
     prop_pos_ext_pos_lit : prop_lit;
     prop_pos_ext_neg_lit : prop_lit;

(* neg extension *)
     prop_neg_ext_pos_lit : prop_lit;
     prop_neg_ext_neg_lit : prop_lit;     
   }

type prop_pred_map = (* pp_map *)
    {
     mutable pp_next_id : int; (* pos_ext = 2*pp_next_id; neg_ext = 2*pp_next_id +1; start wit id=1 *)
     mutable pp_map : prop_pred_ext SMap.t;
     prop_solver : PropSolver.solver;
   }

let create_prop_lit solver sign var_id = 
  PropSolver.create_lit solver (PropSolver.bool_to_sign sign) var_id

let pp_map_add_pred pp_map pred = 
  if (SMap.mem pred pp_map.pp_map)
  then ()
  else
    (
     let pos_ext_id = (2 * pp_map.pp_next_id) in 
     let neg_ext_id = (2 * pp_map.pp_next_id) +1 in 

     pp_map.pp_next_id <-  pp_map.pp_next_id +1;
     let solver = pp_map.prop_solver in
...
(* pos extention *)
	prop_pos_ext_pos_lit = create_prop_lit solver true pos_ext_id;
	prop_pos_ext_neg_lit = create_prop_lit solver false pos_ext_id;
	
(* neg extension *)
	prop_neg_ext_pos_lit = create_prop_lit solver true neg_ext_id;
	prop_neg_ext_neg_lit = create_prop_lit solver false neg_ext_id;
      }
     in
     pp_map.pp_map <- SMap.add pred prop_pred_ext pp_map.pp_map
    )


(*--------*)
let pe_lit_to_prop_lit pp_map (sign,(pred,ext_sign)) = 
  try 
    let pp_ext = SMap.find pred pp_map.pp_map in 
    match (ext_sign,sign) with 
    |(true,true) -> pp_ext.prop_pos_ext_pos_lit
    |(true,false) -> pp_ext.prop_pos_ext_neg_lit
    |(false,true) -> pp_ext.prop_neg_ext_pos_lit
    |(false,false) -> pp_ext.prop_neg_ext_neg_lit
  with
    Not_found -> failwith ("pred_ext_to_lit: pred: "^(Symbol.to_string pred)^" is not in pp_map.pp_map")

let pre_constr_to_clause pp_map pre_constr= 
  List.map (pe_lit_to_prop_lit pp_map) pre_constr

(*-------------------*)

(* mon_process_sub_type: returns true if sub_type can be shown monotone and false otherwise *)

let mon_process_sub_type mon_env sub_type = 
  let pred_set = 
    try 
      SType_Map.find sub_type mon_env.mon_pred_map
    with 
  ...
  dbg_env D_trace 
    (fun () -> 
      let pred_list = SSet.elements pred_set in 
      let pred_str = list_to_string Symbol.to_string pred_list "," in
      dbg D_trace (lazy ("pred_set: "^pred_str^"\n"));
    );

  let sub_type_pre_constraints = 
    try 
      SType_Map.find sub_type mon_env.mon_pre_constraints 
    with 
      Not_found -> []
  in
  let pred_ext_consistency_pre_constraints =  (* ~p_pos \/ ~p_neg *)
    let f pred rest = 
      let pred_constr = [(false,(pred,true));(false,(pred,false))] in
      pred_constr::rest
    in
    SSet.fold f pred_set []
  in
  let pre_constr_all = List.rev_append sub_type_pre_constraints pred_ext_consistency_pre_constraints in
  
  let pp_map = 
    {
     pp_next_id = 1;
     pp_map = SMap.empty;
     prop_solver = PropSolver.create_solver false;
   }
  in 
  let solver = pp_map.prop_solver in

  SSet.iter (pp_map_add_pred pp_map) pred_set;
  ...
  try 
	...
	let prop_clause = pre_constr_to_clause pp_map pre_constr in
	PropSolver.add_clause solver prop_clause
    in
    List.iter f pre_constr_all;

    match (PropSolver.solve solver) with 
    |PropSolver.Sat -> 
	PropSolver.delete_solver solver;
	true
    |PropSolver.Unsat -> 
	PropSolver.delete_solver solver;
	false
...
(*-------monotonox ---*)
  (
   if ( (SSet.is_empty context.pure_var_collapsed_types) && (not (SSet.is_empty context.collapsed_types)))
   then 
     begin
       let start_time_monotx = Unix.gettimeofday () in
       
(*-- used for monotonox-----*)
       let collapsed_types_to_nf_st = ref SMap.empty in

(* -- fill collapsed_types_to_nf_st --*)
       STypeTable.iter
	 (fun nf _st_list ->
	   let nf_type = nf.arg_type in
	   if (SSet.mem nf_type context.collapsed_types)
	   then 
	     let old_nf_list = 
	       try 
		 SMap.find nf_type !collapsed_types_to_nf_st
	       with 
		 Not_found -> [] 
	     in
	     collapsed_types_to_nf_st := SMap.add nf_type (nf::old_nf_list) !collapsed_types_to_nf_st
	   else
	     ()
	 )
	 st_nf_table;
...
(*------monotonox-----*)   
       let mon_env = create_mon_env context in
       mon_process_clause_list mon_env clause_list;

(* if all sub-types of a collapsed type are monotone then the type is monotone *)

       let f collapsed_type st_list = 
	 let g curr_status st = 
	   if curr_status 
	   then
	     mon_process_sub_type mon_env st	
	   else 
	     curr_status (* non-monotone *)
	 in
	 let coll_type_status = List.fold_left g true st_list in
	 if coll_type_status  (* proved monotone *)
	 then
	   (* remove from collapsed *)
	   (
	    context.collapsed_types <- SSet.remove collapsed_type context.collapsed_types;
	    incr_int_stat 1 monotx_restored_types
	   )
	 else ()
       in
       SMap.iter f !collapsed_types_to_nf_st;
       let end_time_monotx = Unix.gettimeofday () in
       assign_float_stat (end_time_monotx -. start_time_monotx) monotx_time;
     end
   else () (* can not do much if context.pure_var_collapsed_types is not empty *)
