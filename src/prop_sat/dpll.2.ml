...
module DPLL_Func_Make(VarLit:VarLit) = 
  struct
...
(*	  state.input_clauses <- old_state.input_clauses; *)
	  ...
	  dbg_env D_reset
		...
		out_str (string_of_int  pr)
	      ...
	      let g v pr = 
		out_str ("v: "^(VarLit.var_to_string v)^" p: "^(string_of_int  pr));
	      in 
...
(* Functional DPLL *)
module DPLL_Func_VInt = DPLL_Func_Make(IntVarLit)

(* let _ = DPLL_Func_VInt.smp_start_smp_dpll () *)
