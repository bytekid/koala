...
exception Unsatisfiable_prop_na
...
(* exception Unsatisfiable *) (* moved to lib.ml *)
...
(* TODO: implement set_decision_var (C++ version has it) *)
...
(** [add_clause s c] asserts the clause [c] given as a list of literals
    ...
    May raise {!Unsatisfiable_gr_na} if the clause set becomes immediately
    unsatisfiable. *)
...
(** Returns next implied unit clause at the 0 decision level; raises Not_found if no new implied clauses *)
...
(* TODO implement set_important_lit; currently only in PicoSAT *)
val set_important_lit : solver -> literal -> unit

(* completely resets solver and removes all caluses; keeps variables *)
val reset_solver : solver -> unit

(* solver should not be used after deletion *)
