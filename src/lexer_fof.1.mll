...
(* eprover starts with fof(c_0_%d *)
  ...
  |'#'[^ '\n']* {
    Format.eprintf "E prover comments"; 
    line proof lexbuf}
  ...
  (* End of file reached *)
  | eof 
      {  Format.eprintf "End of file"; 
	raise End_of_file }

  (* Nothing else allowed *)
  | _

      { Format.eprintf "Non-parsable at line %d"; 
	raise Lexing_error }
...
and parents fof_head fof_id proof fof_parents = parse
  ...
  | "f" (digit+ as parent_id) as fof_cont 
   ...
   (* eprover starts with fof(c_0_%d *)
  | "c_0_" (digit+ as parent_id) as fof_cont
      { 
